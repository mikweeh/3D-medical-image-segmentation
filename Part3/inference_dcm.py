...
def get_predicted_volumes(pred):
    ...
    Arguments:
        pred {Numpy array} -- array with labels. Assuming 0 is background,
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    # genius to make it shine. After all, this is the only part of all our machine learning 
...
def get_series_for_inference(path):
    ...
    series_path = [dir for dir, _, _ in os.walk(path) if 'HCropVolume' in dir][-1]
    dicoms = [pydicom.dcmread(os.path.join(series_path, f)) for f in os.listdir(series_path)]
    ...
    # <YOUR CODE HERE>
    series_for_inference = [dc for dc in dicoms if dc.SeriesDescription=="HippoCrop"]
...
if __name__ == "__main__":
    ...
    # Open the project in the src folder for consistency of relative paths in local test.
    
    # Local test
    local_test = True
    if local_test:
        p = r"../test_vol"
        subdirs = [os.path.join(p, d) for d in os.listdir(p) if
            os.path.isdir(os.path.join(p, d))]
    else:
    ...
    if local_test:
        inference_agent = UNetInferenceAgent(
            device="cpu",
            parameter_file_path=r"../model/c_model.pth")
    else:
        inference_agent = UNetInferenceAgent(
            device="cpu",
            parameter_file_path=r"/home/workspace/model/c_model.pth")
    ...
    if not local_test:
        ...
        os_command("sudo storescu localhost 4242 -v -aec HIPPOAI +r +sd {}".format(report_save_path))
