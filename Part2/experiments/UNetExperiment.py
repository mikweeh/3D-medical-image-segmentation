...
class UNetExperiment:
    ...
    def __init__(self, config, split, dataset):
        ...
        self.val_loader = DataLoader(SlicesDataset(dataset[split["valid"]]),
                batch_size=config.batch_size, shuffle=True, num_workers=0)
    ...
    def train(self):
        ...
        for i, batch in enumerate(self.train_loader):
            ...
            # ANSWER: First dimension points to the image in the batch, second dimension (with value = 1)
            # represents the dimension in which we get the slice, third and forth dimensions are the system
            # coordinates of each voxel in the current slice.

            # Calculate backpropagation and update weights
            loss.backward()
            self.optimizer.step()
